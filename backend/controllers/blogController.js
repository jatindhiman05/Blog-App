...
async function deleteBlog(req, res) {
    try {
            ...
            for (const block of blog.content.blocks) {
                if (block.type === "image" && block.data?.file?.imageId) {
                    try {
                        ...
                        console.error(`Failed to delete image ${block.data.file.imageId}:`, err);
                    }
                }
            }
        }
        ...
        return res.status(200).json({ success: true, message: "Blog and all associated images deleted" });
    } catch (error) {
